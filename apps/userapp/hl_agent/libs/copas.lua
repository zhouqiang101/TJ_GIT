...
function copas.receivefrom(client, size)
  ...
  repeat
    ...
    --print(string.format("---- receivefrom: %s %s %s %s",tostring(client), tostring(s),tostring(err),tostring(port)))
	if err == "timeout" and _closed_skts[client] then
		err = "closed"
		_closed_skts[client] = nil
		--print(string.format("copas.receivefrom skt(%s) set err to closed",tostring(client)))
	end
...
local _skt_mt_tcp = {
                   ...
                   close = function(self, ...) 
						if _reading_log[self.socket] or _writing_log[self.socket] then
...
_skt_mt_udp.__index.close       = function(self, ...) 
						            if _reading_log[self.socket] or _writing_log[self.socket] then
								    	_closed_skts[self.socket] = true
									end
									return true
...
local function debug_print_stat()
	function cnt(t)
		local c = 0;
		for i in pairs(t) do
			c = c + 1
	...
	print(string.format("_rl:%d,_wl:%d,_cs:%d,_r:%d,_w:%d,_t:%d",cnt(_reading_log),cnt(_writing_log),cnt(_closed_skts),cnt(_reading),cnt(_writing),cnt(_tasks)))
end
