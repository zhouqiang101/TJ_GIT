...
local assert = function(x, err)
    --if not x then print(debug.traceback()) end
...
_M.connect = function(self, host, port, connopts)
    ...
    if connopts.usessl then
        ...
        local params = { mode = "client", protocol = "tlsv1", verify = "none" }
...
_M.setkatimeout = function(self)
    self.keepalive_timer = timer.new(self.opts.keep_alive or 20)
end
...
local pingresp_timeout_times = 0
...
_M.cycle_packet = function(self)
    ...
    elseif type == self.PINGRESP then
		...
		pingresp_timeout_times = 0
    end

    if self.pingresp_timer then
		if self.pingresp_timer:remain() <= 0 then
			pingresp_timeout_times = pingresp_timeout_times + 1
            self.pingresp_timer = timer.new(8)
			if pingresp_timeout_times >= 3 then
        		assert(nil, "can not receive ping response")
			end
		end
    end
    self:keepalive()
